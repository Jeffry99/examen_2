...
import org.una.examen.provincias.services.IUnidadService;
...
public class UnidadController {
    ...
    public ResponseEntity<?> findByNombre(@PathVariable(value = "term") String term) {
        try {
            return new ResponseEntity(unidadService.findByNombre(term), HttpStatus.OK);
        } catch (Exception e) {
    ...
    @PostMapping("/crear") 
    @ResponseBody
    public ResponseEntity<?> create(@RequestBody UnidadDTO canton,  BindingResult bindingResult) {
        if (!bindingResult.hasErrors()) {
            try {
                return new ResponseEntity(unidadService.create(canton), HttpStatus.CREATED);
            } catch (Exception e) {
                return new ResponseEntity<>(e, HttpStatus.INTERNAL_SERVER_ERROR);
            }
        } else {
    ...
    @ResponseBody
    public ResponseEntity<?> update(@PathVariable(value = "id") Long id, @RequestBody UnidadDTO unidadModificada, BindingResult bindingResult) {
        if (!bindingResult.hasErrors()) {
            try {
                Optional<UnidadDTO> unidadUpdated = unidadService.update(unidadModificada, id);
